...
namespace mysqlconnector
{
     public static class Extensions
    {
        ...
        public static IEnumerable<T> HydrateRows<T>(this IDataReader reader) where T : new()
        {
            ...
            while (reader.Read())
            {
                T newObject = reader.HydrateFields<T>(hashtable);
                entities.Add(newObject);
            }
            return entities;
        }

        public static T HydrateFields<T>(this IDataReader reader, System.Collections.Hashtable properties = null) where T : new()
        {
            T entity = new T();
            if (properties == null) properties = GetProperties<T>();
